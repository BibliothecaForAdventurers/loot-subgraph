...
export function initWallet(from: Address, event: ethereum.Event): Wallet {
  let fromAddress = from;

  let fromId = fromAddress.toHex();
  let wallet = Wallet.load(fromId);
  if (!wallet) {
    wallet = new Wallet(fromId);
    wallet.address = fromAddress;
    wallet.joined = event.block.timestamp;
    wallet.raidAttacks = BigInt.fromI32(0)
    wallet.raidDefends = BigInt.fromI32(0)
    wallet.realmsHeld = BigInt.fromI32(0);
    wallet.srealmsHeld = BigInt.fromI32(0);
  }
  return wallet
}
